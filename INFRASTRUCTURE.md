...
- **Smoke-test the Qualtrics integration end-to-end (after team decisions).** Three secrets must be set first in Supabase → Project Settings → Edge Functions → Secrets:
  - `PARTNER_API_KEY_QUALTRICS` — ✅ **set 2026-08-15/16** (see the Draft 84 change-log entry). ⚠️ **This line previously contained a generated key in cleartext (committed 2026-05-06, `4862350`). That value is BURNED — it is permanently in git history and must never be installed.** It was never actually the live secret (Draft 76's harness proved the secret was unset until Josh generated a fresh one), so there is nothing to rotate *here* — but if that string was reused for anything else, rotate that. Never paste a real key into this file or `docs/QUALTRICS_SETUP.md`; the live value lives only in the Supabase secret store and the Qualtrics credential store.
...
- **2026-08-16** — **`mint-access-code` v3: the shared secret is now accepted from `Authorization` as well as `x-partner-key`** (Draft 84, P0 — the last blocker on the consent → access-code pipeline). **Deployed via CLI with `verify_jwt` DISABLED** (`npx supabase functions deploy mint-access-code --no-verify-jwt --project-ref fflezknnpmbemeqyqxml`); deployed version is now **6**, `verify_jwt` confirmed still `false` after the deploy. `x-partner-key` remains the documented primary and wins when both headers are present; `Authorization` is read as a fallback with an optional `Bearer ` prefix stripped (case-insensitive) and the value trimmed (an invisible trailing newline pasted into a credential field would otherwise surface only as a bare "Invalid partner key" — trimming can remove whitespace but can never match a different secret). The `!==` comparison is unchanged (constant-time comparison explicitly out of scope). **Why:** the Qualtrics Workflow WebService task's stored "API key" credential always sends `Authorization: <token>` and its editor exposes only Name / API Token / mTLS — the header name cannot be changed on the Qualtrics side, and putting the raw key in a plaintext custom header was rejected in favor of the credential store. **The architectural backstory worth remembering:** Qualtrics **Survey Flow web service elements never fire on real submissions here** — two live anonymous-link submissions with consent = Yes were recorded and this function received *zero* requests (not a 401, not a 500; the connection was never opened), with placement/config/publish state all verified correct. The Survey Flow surfaces no execution logs, which is why it went unnoticed for weeks. Minting therefore moved to a **Qualtrics Workflow** (`WF_lFfvg4FT5Ltm9SA`, which does have per-run history): survey-response event → consent decision → authenticated WebService POST. That is why two mechanisms appear in this history, and why **the two dead Survey Flow elements must be deleted** — if they ever started firing alongside the Workflow, every consent would mint four codes instead of two. Verified: the Supabase gateway passes non-JWT `Authorization` values straight through to a `verify_jwt=false` function (probes with a raw token, `Bearer <garbage>`, and even `Bearer <public anon JWT>` all reached the function and returned its own 401, never a gateway error); 22/22 local header-resolution checks (incl. the public anon JWT NOT authorizing, precedence holding even when `x-partner-key` is wrong, only one `Bearer` stripped, `Basic` rejected); post-deploy regression probe — all six unauthorized header shapes still 401 and no `access_codes` rows created. Also corrected `docs/QUALTRICS_SETUP.md` §2 (it still instructed building the web service elements that don't fire) and the stale secrets-checklist row (`PARTNER_API_KEY_QUALTRICS` is now confirmed **set** — wrong keys 401 instead of 500). **Still Qualtrics-side and NOT done here:** the second WebService task for `rsd-follow-up-90d`, writing the four code/url fields back to embedded data, the two consent-time emails, deleting the dead Survey Flow elements, and publishing/enabling both workflows (`WF_lFfvg4FT5Ltm9SA` and the completion-webhook receiver `WF_JFIOeoc0oOU3G4T`, which is also still Draft/Disabled).
  - **⚠️ Known confidentiality cost of the Authorization path, found by the adversarial review and confirmed by measurement — `Authorization` is second-best, use `x-partner-key` wherever the header name can be set.** The gateway does not *enforce* Authorization on a `verify_jwt=false` function, but it does *parse* it: a value it can't recognize as a Supabase key is written to `function_edge_logs` as `request.sb.apikey.authorization.prefix` — **the first 10 characters of the secret** — with `...authorization.error = "invalid"`. Measured with a 32-char sentinel (`SENT2BBBBB…` logged exactly 10 chars, so ~22 of 32 stay secret); `x-partner-key` appears nowhere in the log schema. So every Qualtrics consent that authenticates via Authorization deposits a partial copy of the live secret into project logs, readable by any project member or management token, for the log-retention window. **Not a bypass** (~130 bits remain; the 1000/day post-auth cap still bounds abuse) — but the Authorization-borne key is lower-trust material and should be on a rotation schedule. Remediations shipped with this entry: the misleading "Supabase never tries to interpret that Authorization value" claim in the function header was corrected to state the logging behavior; `scripts/qualtrics-smoke.mjs` now sends the REAL key only via `x-partner-key` (its Authorization coverage is negative-only — a wrong token and the public anon JWT, neither secret), so harness runs no longer deposit a prefix; and both QUALTRICS_SETUP §2 and the function header now say to prefer `x-partner-key` wherever the name is settable. If the two paths should be independently revocable, accept a second distinct secret for Authorization so a prefix leak is scoped to the Qualtrics credential.
  - **🔑 Credential hygiene, also from the review:** the "Open follow-ups" section of THIS file carried a **generated partner key in cleartext** (committed 2026-05-06, `4862350`) and read as a live instruction to install it. It has been replaced with a placeholder and marked burned. It was never the live secret (Draft 76 proved the secret was unset until Josh generated a fresh one), so there is nothing to rotate for this system — **but the string is permanently in git history and must never be installed**; rotate anywhere else it may have been reused.
  - **Runbook corrections (the review caught these as revert-bait):** `docs/QUALTRICS_SETUP.md` §8.2 still prescribed renaming the credential parameter to `x-partner-key` as "the fix" — advice that is now both unnecessary and impossible in the Workflow editor, and the single place a future debugger grepping `"Invalid partner key"` would land. §8.2/§8.3 are now explicitly marked HISTORICAL with pointers to §2/§9/§10, §8's heading warns that the later-numbered sections supersede it, and §8.1's "✅ Built + proven live" for the mint calls was corrected — those two real codes came from builder **Test** clicks, which proved the endpoint, not the delivery.
- **2026-08-15 (night, later)** — **Locked-instrument alignment, decision-independent slice authored for v6** (Draft 83; builder tables only — **no publish, live v5 untouched and verified to carry none of the new fields**). From the locked Pretest/Posttest docs (Draft 5.11.26, extracted from the .docx XML): **verbatim `anchor_labels` on the core scales** — BHS 0–3 ("Absolutely disagree…Absolutely agree", pre `106fef64` + post `983ed115`), ASCS 1–5 ("Never…Always", pre `3b0077de` + post `aee35777`), UCLA 1–3 ("Hardly ever/Some of the time/Often", pre `5cc6837a`), NB 1–5 ("Strongly disagree…Strongly agree", labels as printed with reverse flags untouched on nb1/nb2, pre `e10a0017` + post `4032db48`), BPB 0–3 ("Never/Sometimes/Often/Always", pre `5d2b2245`) — and the **bw1→bw2 `show_if` conditional + skip note on both BW pairs** (pre `d18ee378` + post `bd56a665`), byte-identical to follow-up v2's config (SQL-asserted). All edits via `jsonb_set` so they compose with the pending Draft 71/72 builder edits. Scope guard honored: appraisals (Q1), program helpfulness/acceptability (Q2), demographics/PDW (Q3) untouched. Builder-vs-published-v5 diff moved from the 11 prior intended rows to exactly 16 (the 5 newly-differing: self_agency_pre/post, loneliness_pre, belong_stress_pre/post; the other 5 in-scope items were already in the diff from D71/72 and just changed hash). Verified: label array lengths match each scale's range with verbatim endpoints (SQL), and rendering checked at a real 375px viewport by temporarily mounting the exact authored rows in the sandbox (labels + echo + aria-labels, no overflow; BW note at 0 / bw2 appears above 0) — throwaway entries reverted, nothing shipped. **Noted gap:** the locked BW header prints a mid-anchor ("Moderately" at ~5) that the VAS renderer can't show (min/max labels only) — a tiny `vas_config.mid_label` capability if the team wants it, not authored here. Effective at the v6 republish.
- **2026-08-15 (night)** — **Completion-webhook delivery: outcomes recorded + re-fire path** (Draft 82). **`update-session-progress` bumped to v3** (MCP deploy, `verify_jwt` stays true — the client sends the anon JWT). Three changes: (1) every completion's webhook outcome now persists to `sessions.metadata_json.webhook` — `{status: 'delivered'|'failed'|'skipped', at, attempts, reason?, last_error?, last_http_status?}`; `skipped` covers the two previously-silent cases (`no_external_ref` on QA/admin codes; `webhook_not_configured` while the URL secret is unset). (2) Retry hardened: up to **3 attempts with 2s/4s backoff** on 5xx/network, **never on 4xx** (a rejection won't succeed on retry), and the whole send+record now runs in the background via `EdgeRuntime.waitUntil` — v2 awaited the webhook inline, so a flaky receiver could add seconds to the participant's completion POST; v3 responds immediately (measured 414–510ms with the background record landing ~1s later). (3) New **`scripts/refire-webhook.mjs <session_id> [--dry-run]`** rebuilds the exact original payload from the session row, re-POSTs with the same retry algorithm, and updates the record (`refired: true`); requires `SUPABASE_SERVICE_ROLE_KEY` + `QUALTRICS_COMPLETION_WEBHOOK_URL` in env. The recovery procedure at study scale is the one query in the script header (completed + external_ref + record not 'delivered' — also catches pre-v3 completions with no record) plus that script; no queue, no admin UI. **Idempotency caveat verified against the runbook receiver design:** Qualtrics-side flag-setting is idempotent, but the confirmation EMAIL could re-send on a true duplicate — the runbook now recommends conditioning email/incentive steps on the flag not already being set, and the script warns before re-firing an already-delivered record. Verified: delivery algorithm 14/14 local checks (200 → delivered/1; 500×3 → failed/3 with measured 2s+4s backoff; 400 → failed/1 no retry; network error → failed/3); both `skipped` reasons live in production (temp codes, one with a SQL-authored `external_ref`, both sessions completed on the deployed v3 and recorded correctly); re-fire script 10/10 against a REST stub serving the real session rows (payload byte-shape, PATCH record, exit codes, --dry-run). Full `delivered`/`failed` against real Qualtrics remains Thursday's joint test (URL secret still unset — unchanged). QUALTRICS_SETUP.md updated (§4 v3 semantics + recovery section, §7 delivery-record check). + `rsd-follow-up-90d` republished as v2** (Draft 78). Part A enumeration of the three locked instruments in `Final Measures/` found exactly ONE display-logic conditional, appearing in all three (Pretest, Posttest, FollowUp): "To what degree do you have worries about belonging…? *(If they select "0" on the slider, Q2 will be skipped)*" — bw1 gates bw2. (The FollowUp permanency "Other: (please specify)" is an inline specify affordance on a choice option, not display logic — the current choice + optional free_text authoring already records it; not built.) Because bw1/bw2 live INSIDE one `psychometric_scale` item's `items` array, the engine-level item `show_if` originally sketched can't express it — the smallest correct capability is a **sub-item `show_if`** in `PsychometricScale`: `{ item_id, operator (equals|not_equals|gt|gte|lt|lte|in), value }` referencing another item in the same scale, evaluated live against the in-scale responses. Three states: gate unanswered → dependent renders nothing (progressive disclosure); condition met → normal item; condition failed → the optional authored `skip_note` renders in its place (styled like the survey-mirror skip notice, `aria-live=polite`). Hidden/pending items are excluded from the Continue gate, and **responses are pruned to visible items at save** (an answer given before an upstream change never rides along as an orphan). Malformed conditions fail OPEN (item shows — a broken condition must never silently drop a locked-instrument question). Export verified, not assumed: `exportFlatten` emits `''` for an absent `scale_responses` key, so a skipped bw2 lands as an SPSS missing cell while the column stays in the codebook. Builder round-trips the new fields untouched (`patchItem` spreads). Authoring: bw2's `show_if` (gt 0) + skip_note written to the builder item, **v2 published** (snapshot assembled matching `assembleSnapshot`; v1→v2 diff verified to be exactly the one item), `current_version_id` flipped. Verified live at 375×812 on two temp single-use codes, both completed on v2: bw1=3 → bw2 appears, payload `{bw1:3, bw2:7}`; bw1=0 → skip note, Continue enabled, payload `{bw1:0}` with no bw2 key, 12/12 responses. Orphan-prune also proven in the sandbox (bw2 answered, bw1 moved to 0, save carries only bw1). Sandbox QA surface: `/demo/sandbox/scale-conditional-preview` (unlisted). No schema/DDL or edge-function change; no activity version bumps (engine + authoring).
- **2026-08-15 (later still)** — **Rate limiting on the public edge functions** (Draft 77; carried in the deferred list since May, bought now ahead of code distribution). New migration `edge_rate_limits`: a service_role-only counters table (`bucket` PK = `<function>:<key>:<YYYY-MM-DD>`, so counters reset daily by construction) + an atomic `bump_rate_limit(bucket)` upsert-increment SQL function (EXECUTE granted to service_role only; RLS enabled with no policies — invisible to the public Data API). Every limiter **fails OPEN** on counter errors — a counter hiccup must never block a participant. Deployed caps: `validate-code` **v4** — 100/day/IP, counted on every request BEFORE the code lookup (bounds brute-forcing), 429 with kid-friendly copy ("Too many tries — take a break and try again later, or ask your caregiver for help."); `submit-feedback` **v6** — 40/day/IP; `get-rsd-snapshot` **v2** — 200/day/IP (note: this MCP redeploy flipped its `verify_jwt` false→true, same landmine as submit-feedback v5 — verified harmless since /demo sends the anon JWT; bare unauthenticated GETs now 401, a tightening). **`mint-access-code` was deliberately NOT redeployed via MCP** — an MCP deploy would flip `verify_jwt` to true and break Qualtrics (which sends no JWT) ahead of the partner-key check. Its rate-limited **v2 source is repo-tracked at `supabase/functions/mint-access-code/index.ts`** (per-PARTNER-KEY cap of 1000/day counted post-auth — Qualtrics batches can share one egress IP, so IP-keying could throttle legitimate consent bursts) and must be deployed by Josh via `supabase functions deploy mint-access-code --no-verify-jwt`. Verified live: normal validate/feedback/snapshot flows unaffected; counter seeded to 99 → the 100th validate passed and the 101st returned the 429 + copy; bucket delete restored normal flow immediately.
- **2026-08-15 (later)** — **Qualtrics integration runbook + smoke harness** (Draft 76). New `docs/QUALTRICS_SETUP.md`: the complete Qualtrics-side build guide in survey-flow order (embedded-data fields, the two mint Web Service calls with exact request/response JSON + piped-text syntax, the two triggered emails with the Outlook-safe button pattern, the completion-webhook JSON-event receiver + workflow branching on `intervention_slug`, the 90-day scheduled email, secrets checklist, and the joint end-to-end test script) — contracts verified against deployed sources `mint-access-code` v1 / `validate-code` v3 / `update-session-progress` v2. New `scripts/qualtrics-smoke.mjs`: simulates Qualtrics against the LIVE functions — mint auth probes, mint round-trip (when the partner key is in env), validate → resume-by-code → complete → completed-routing per slug, webhook status notes, cleanup SQL. **Harness run 2026-08-15: all 16 functional checks green on both slugs** (including the Draft 75 follow-up), retiring the our-side half of the smoke test pending since May. **One blocking discovery: `PARTNER_API_KEY_QUALTRICS` is NOT actually set** — mint returns 500 "Server misconfigured" (the env-var-missing branch), despite this doc's May note saying it was handed off. Josh must set it in the Supabase dashboard before the Thursday Qualtrics build; the runbook's secrets checklist reflects this. Remaining for Thursday's joint test: the webhook URL/token secrets + the runbook §7 script.
- **2026-08-15** — **`rsd-follow-up-90d` created, authored, and published as v1** (Draft 75; the slug was allow-listed in `mint-access-code` and routed in the participant flow but the intervention never existed — follow-up links would have dead-ended). Structure: 5 sections / 12 items — Welcome (locked FollowUp intro from `FollowUp Survey Draft Belongingness_5.2.26.docx`, the doubled-word "of this of the" typo corrected pending sign-off), "How you feel" (BHS 0–3, ASCS 1–5, UCLA 1–3, NB 1–5 with reverse flags on nb1/nb2), "Your skills and thoughts" (BPB 0–3, the 6 locked Appraisals 0–4, Belonging Worries as two 0–10 VAS), "Your placement" (permanency `choice` + optional other-text `free_text` + disruption worry 0–4), "All done" (thanks + a **per-intervention celebration override** via the last section's `config_json.celebration` — new capability in `DeliveryShellPage`, defaults preserve the main program's Draft 74 copy). Every likert uses Draft 73 `anchor_labels` (sparse on Appraisals: only 0/2/4 labeled, matching the locked doc). Token keys `*_fu` mirror the pre/post convention. Snapshot assembled in SQL matching `builderUtils.assembleSnapshot` field-for-field; `current_version_id` flipped. Known limitations flagged: no bw2 conditional skip (renderer limitation, same as live pre/post), permanency other-text as a separate optional item, no re-assent (per flow doc — confirm with Jessica). Verified end-to-end with a temp single-use code at 375×667 (all items, labels, celebration override, completion + 12/12 responses). **Also: `validate-code` bumped to v3** — QA caught a real race: the `?code=` auto-submit double-fired (StrictMode double-effect; same race exists for real double-taps), minting two sessions 21ms apart on a single-use code, and resume picked the empty newest-started twin. Fix: resume lookup now orders by `last_active_at DESC` (the worked-in session is always fresher — save-response and update-session-progress both touch it), and `CodeEntryPage`'s auto-submit is ref-guarded against double-fire. Reconciliation verified live with the twin present.
- **2026-08-14** — **Resume-by-code** (Draft 69; fixes QA_MOBILE_2026-08 P0-1). `validate-code` bumped to **v2** with a contract change: for **single-use codes (`max_uses = 1`)**, re-entering the code (or re-clicking the emailed `?code=` link) now returns the participant's **existing session** instead of inserting a new one — response gains `resumed: boolean` and, on resume, `session_status` (`in_progress` resumes at `current_section` with responses hydrated; `completed` routes to the engine's already-finished screen). No insert, no `use_count` bump on resume: `use_count` now counts *sessions created*, and `max_uses = 1` means "one participant," not "one browser tab, ever." **Multi-use codes keep the old mint-per-validation behavior deliberately** (that's what internal QA codes want); abandoned sessions are deliberately not resumed (releases the code to the exhausted check). Intervention `is_active`/published gates are not applied to a resume (the session's `version_id` is frozen; a mid-study deactivation shouldn't strand a kid). Engine-wide — GAINS codes get identical behavior. Client side: `CodeEntryPage` flags a resumed in-progress session via a one-shot `sessionStorage` key; `DeliveryStepPage` shows a "Welcome back — picking up where you left off." banner that clears on the first advance (flag read/removal split across `useState`/`useEffect` — removing inside the initializer breaks under StrictMode's dev double-mount). Verified live end-to-end with a temp single-use code: create → partial progress → simulated browser close → re-entry by typed code AND by `?code=` link both return the same session at the right position with `use_count` still 1; completion → re-entry shows the friendly finished screen; multi-use `TEST-RSD-001` still mints distinct fresh sessions. **Documented behavior change:** two tabs entering the same single-use code now share one session (last-write-wins per item on the response upsert) — previously impossible by construction, accepted as far better than the lockout it replaces. No schema/DDL change.
- **2026-07-22 (later)** — Assent copy corrected to be **verbatim** from the IRB doc and **republished as version 4** (`c39b0b21-…`, `current_version_id` flipped). The v3 copy had a friendly invented heading and lightly-edited decision wording; v4 uses the document's exact 4-line title (`text_prompt.heading` = "A SINGLE SESSION PROGRAM TO FOSTER BELONGINGNESS"; the other three title lines lead the body), the body word-for-word, the decision line's exact wording (lowercase "yes", "No" with the period outside the quote), and plain **Yes** / **No** choice labels. Content was pushed via `execute_sql` with the text **base64-decoded** in-SQL (`convert_from(decode(...,'base64'),'UTF8')`) to eliminate any quoting/escaping drift, then verified byte-identical to the source (2451-char body) both in the draft and in the published snapshot. Only invisible Word artifacts (non-breaking spaces → normal spaces, trailing whitespace) were normalized. No structural change from v3 (still 14 sections / 57 items). Sandbox mirror bumped to `assent` v1.1.
- **2026-07-22** — Child Assent added as the first step of the live **Ready! Set! Dedicate!** intervention (`ready-set-dedicate`, id `29294f95-…`). Content authored directly into the Supabase draft (`sections`/`items`) via `execute_sql`: a new `intro` section titled **"Assent"** inserted at `order_index 0` (all 13 existing sections shifted to 1–13, otherwise unchanged), holding two items — a `text_prompt` with the IRB assent body (verbatim from `Belongingness_Assent_ages 6-11_updated7.22.26.doc`) and a Yes/No `choice` (token_key `assent`) whose `content_json.exit_on` ends the session on **No** with a friendly exit screen (engine's existing hard-exit mechanism, `SessionEngine.matchesExitRule`). Confirmed the draft was in sync with live v2 before publishing (no post-publish draft edits), then **published version 3** — snapshot assembled in SQL matching `builderUtils.assembleSnapshot` exactly (14 sections / 57 items; v2 was 13/55) — and flipped `interventions.current_version_id` to the new version (`6e861fd0-…`). New sessions (`validate-code` freezes `version_id = current_version_id`) now open on the assent; in-progress sessions keep their frozen version. No schema/DDL change (content + version rows only); no edge-function change. Sandbox mirror shipped alongside (`src/activities/Assent.jsx` v1.0, surfaced first on `/demo`). **Verification note:** the live `/session` render wasn't clicked through (needs a real access code); the published snapshot was verified structurally (assent first, `exit_on` intact) and the sandbox component was fully exercised (both Yes/No paths + exit screen).
