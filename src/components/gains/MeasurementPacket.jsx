// GAINS measurement packet (Draft 53, 2026-08-27) — the Child Assent &
// Measures section, rendered as the actual questionnaires for team review.
//
...
//
// Scope, per the draft: this is for team review on the demo page (which
...
// ---------- small local layout pieces ----------

function GroupHeading({ children }) {
  return (
    <h3
      className="text-[13px] font-bold uppercase mt-8 mb-3 first:mt-0"
      style={{ letterSpacing: 'var(--tracking-caps)', color: 'var(--text-warm)', fontFamily: 'var(--font-core)' }}
    >
      {children}
    </h3>
  )
...
function Instrument({ title, timing, prompt, note, children }) {
  return (
    <div
...
export default function MeasurementPacket() {
  const [v, setV] = useState({})
  const set = (key) => (val) => setV((prev) => ({ ...prev, [key]: val }))

  return (
    <div className="max-w-[560px]">
      <GroupHeading>Pre-only</GroupHeading>

      <Instrument title="Demographics" timing="Pre">
      ...
      <Instrument title="Event: time since trauma" timing="Pre">
        <Item
          n={null}
      ...
      <Instrument title="Therapy history (present & past)" timing="Pre">
      ...
      <GroupHeading>Pre + Post</GroupHeading>
      ...
      <Instrument title="Motivation / Readiness to Change Ruler" timing="Pre + Post">
      ...
      <Instrument
        title="Implicit Theories of Emotion Scale – Child Version"
        timing="Pre + Post"
      >
        {IMPLICIT_THEORIES_ITEMS.map((text, i) => (
          <Item key={i} n={i + 1} text={text}>
            <LikertScale
      ...
      <Instrument title="Trauma and Treatment Beliefs" timing="Pre + Post">
        {TRAUMA_BELIEFS_ITEMS.map((item, i) => (
      ...
      <GroupHeading>Post-only</GroupHeading>

      <Instrument title="Program Feedback Scale" timing="Post">
